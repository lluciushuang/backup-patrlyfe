...
<div class="page-wrapper">
    <div class="breadcrumb">
        <a href="{{ route('home') }}">HOME</a> <span>/</span> <a href="{{ route('transactions.index') }}">TRANSAKSI</a> <span>/</span> INVOICE
    </div>
    ...
    <div class="co-section" style="background: var(--surface-2); max-width: 800px; margin: 2rem auto; padding: 2.5rem;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 2.5rem; flex-wrap: wrap; gap: 1.5rem;">
            ...
            <div style="text-align: right;">
                ...
                <p style="font-family: var(--font-mono); color: var(--gray-mid); margin-bottom: 0.5rem;">{{ $transaction->invoice_number }}</p>
                <span style="display: inline-block; padding: 0.35rem 0.85rem; background: {{ $statusColor }}20; color: {{ $statusColor }}; border-radius: 4px; font-size: 0.75rem; font-weight: 600;">{{ $statusText }}</span>
            </div>
        </div>
        ...
        <div style="display: flex; justify-content: space-between; margin-bottom: 2.5rem; flex-wrap: wrap; gap: 1.5rem; padding-bottom: 1.5rem; border-bottom: 1px solid rgba(242,239,230,0.08);">
            <div>
                ...
                <p style="font-weight: 600; color: var(--off-white); margin-bottom: 0.25rem;">{{ Auth::user()->name }}</p>
                <p style="color: var(--gray-mid); font-size: 0.85rem; margin-bottom: 0.15rem;">{{ Auth::user()->phone ?? '-' }}</p>
                <p style="color: var(--gray-mid); font-size: 0.85rem;">{{ Auth::user()->email }}</p>
            </div>
            <div style="text-align: right;">
                ...
                <p style="color: var(--off-white); font-size: 0.85rem;">{{ $transaction->created_at->timezone('Asia/Jakarta')->format('d F Y H:i') }} WIB</p>
            </div>
        </div>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 2.5rem;">
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid rgba(242,239,230,0.05);">
                    <td style="padding: 1rem 0.5rem; color: var(--off-white);">{{ $detail->product->name ?? 'Produk tidak tersedia' }}</td>
                    <td style="padding: 1rem 0.5rem; text-align: center; color: var(--gray-mid);">{{ $detail->qty }}</td>
                    <td style="padding: 1rem 0.5rem; text-align: right; font-family: var(--font-mono); color: var(--gray-mid);">Rp {{ number_format($detail->price, 0, ',', '.') }}</td>
                    <td style="padding: 1rem 0.5rem; text-align: right; font-family: var(--font-mono); color: var(--off-white);">Rp {{ number_format($detail->qty * $detail->price, 0, ',', '.') }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div style="display: flex; justify-content: flex-end;">
            <div style="width: 280px;">
                <div style="display: flex; justify-content: space-between; padding: 0.75rem 0; border-bottom: 1px solid rgba(242,239,230,0.08);">
                    ...
                    <span style="font-family: var(--font-mono);">Rp {{ number_format($transaction->total_amount, 0, ',', '.') }}</span>
                </div>
                <div style="display: flex; justify-content: space-between; padding: 1.25rem 0; margin-top: 0.5rem; border-top: 2px solid var(--orange);">
                    ...
                    <span style="font-family: var(--font-mono); font-size: 1.3rem; color: var(--orange);">Rp {{ number_format($transaction->total_amount, 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
