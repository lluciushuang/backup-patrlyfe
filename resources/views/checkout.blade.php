...
<div class="checkout-layout">
    <div>
        ...
        <div class="co-section">
            ...
            <div class="addr-card">
                <div style="font-weight:600; font-size:0.95rem;">{{ Auth::user()->name ?? 'Pelanggan' }}</div>
        ...
        <div class="co-section">
            ...
            @if(!empty($checkoutItems))
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.75rem 0; border-bottom: 1px solid rgba(242,239,230,0.05);">
                    <div style="flex: 1;">
                        <div style="font-weight: 500; color: var(--off-white); font-size: 0.9rem;">{{ $item->name }}</div>
                        <div style="font-size: 0.75rem; color: var(--gray-mid); font-family: var(--font-mono);">
                            @if($item->original_price && $item->original_price > $item->price)
                                <span style="text-decoration: line-through; color: var(--gray-mid);">Rp {{ number_format($item->original_price, 0, ',', '.') }}</span>
                                &nbsp;<span style="color: var(--orange);">Rp {{ number_format($item->price, 0, ',', '.') }}</span>
                            @else
        ...
        <div style="background: rgba(232, 82, 26, 0.05); border: 1px solid rgba(232, 82, 26, 0.2); border-radius: 4px; padding: 0.75rem 1rem; margin: 1rem 0; font-size: 0.85rem;">
            <span style="color: var(--orange); font-weight: 600;">Level Harga {{ $tierPriceLevel }} berlaku</span>
    ...
    <div>
        <div class="co-section" style="background:var(--surface-2);">
            ...
            @endphp
            @if($tierPriceLevel > 1 && $originalTotal > $total)
            <div style="display:flex; justify-content:space-between; font-size:0.85rem; margin-bottom:0.5rem;">
                <span style="color:var(--gray-mid);">Subtotal</span>
                <span style="font-family: var(--font-mono); text-decoration: line-through;">Rp {{ number_format($originalTotal, 0, ',', '.') }}</span>
            </div>
            <div style="display:flex; justify-content:space-between; font-size:0.85rem; margin-bottom:0.5rem;">
                <span style="color:var(--gray-mid);">Hemat (Level {{ $tierPriceLevel }})</span>
                <span style="font-family: var(--font-mono); color:var(--green);">-Rp {{ number_format($discountAmount, 0, ',', '.') }}</span>
            </div>
            @endif
            <div style="display:flex; justify-content:space-between; font-size:0.85rem; margin-bottom:0.5rem;">
                <span style="color:var(--gray-mid);">Total Items ({{ is_array($checkoutItems) ? array_sum(array_column($checkoutItems, 'qty')) : ($checkoutItems->sum('qty') ?? 0) }} barang)</span>
                <span style="font-family: var(--font-mono);">Rp {{ number_format($total ?? 0, 0, ',', '.') }}</span>
            </div>
            <div style="display:flex; justify-content:space-between; font-family:var(--font-display); font-size:1.5rem; border-top:1px solid rgba(242,239,230,0.08); padding-top:0.75rem; margin-top:0.75rem;">
                <span>Total</span>
                <span style="font-family: var(--font-mono); color: var(--orange);">Rp {{ number_format($total ?? 0, 0, ',', '.') }}</span>
            </div>
            @if(!empty($checkoutItems))
            ...
            if (response.status === 'success') {
                snap.pay(response.snap_token, {
                    onSuccess: function(result) {
                        ...
                        setTimeout(() => window.location.href = '{{ route("transactions.index") }}', 1500);
                    },
