...
<div class="page-wrapper">
    <a href="{{ route('admin.orders.index') }}" class="btn btn-outline btn-sm back-btn">
        ← KEMBALI KE PESANAN
    </a>
                    ...
                    <div class="product-item">
                        <div class="product-img-sm">
                            @if($detail->product->images->first())
                                <img src="{{ asset('storage/' . $detail->product->images->first()->image_path) }}" alt="{{ $detail->product->name }}">
                            @else
                        ...
                        <div style="flex: 1;">
                            <div style="font-weight: 600; color: #F2EFE6; margin-bottom: 0.25rem;">{{ $detail->product->name ?? 'Produk tidak ditemukan' }}</div>
                            <div style="font-size: 0.75rem; color: #888880; margin-bottom: 0.5rem;">{{ $detail->product->brand ?? '-' }}</div>
                            <div style="display: flex; justify-content: space-between; font-size: 0.85rem;">
                                <span style="color: #888880;">Qty: {{ $detail->qty }} x Rp {{ number_format($price, 0, ',', '.') }}</span>
                                <span style="color: #E8521A; font-weight: 600;">Rp {{ number_format($detail->price * $detail->qty, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        <aside>
            <div class="order-card">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value" style="font-family: monospace;">{{ $order->invoice_number }}</span>
                </div>
                <div class="info-row">
                ...
                <div class="info-row" style="border-top: 1px solid rgba(242, 239, 230, 0.1); margin-top: 0.5rem; padding-top: 1rem;">
                    ...
                    <span class="info-value" style="font-size: 1.2rem; color: #E8521A;">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</span>
                </div>
            </div>

            <div class="order-card" style="margin-top: 1.5rem;">
                <h2 class="section-header">Ubah Status</h2>
                <select onchange="updateOrderStatus({{ $order->id }}, this.value)" style="width: 100%; padding: 0.75rem; background: #161614; border: 1px solid rgba(242, 239, 230, 0.1); color: #F2EFE6; border-radius: 2px;">
...
function updateOrderStatus(orderId, status) {
    const url = '{{ route('admin.orders.update-status', ':id') }}'.replace(':id', orderId);
