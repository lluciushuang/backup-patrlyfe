                ...
                <tr>
                    ...
                    <td>
                        @if($product->images->count() > 0)
                            <img src="{{ asset('storage/' . $product->images->first()->image_path) }}" alt="{{ $product->name }}" class="product-image-thumb" onerror="this.style.display='none';">
                        @else
                    ...
                    <td>
                        <span class="stock-badge {{ $product->current_stock == 0 ? 'stock-out' : ($product->current_stock <= 5 ? 'stock-low' : 'stock-ok') }}">
                            {{ $product->current_stock }} pcs
                        </span>
                    </td>
                    <td>
                        @php $price = $product->prices->where('price_level', 1)->first(); @endphp
                        Rp {{ number_format($price ? $price->price : 0, 0, ',', '.') }}
                    </td>
                    <td>
                        <button class="btn btn-sm" style="background:transparent; border:1px solid var(--orange); color:var(--orange);" onclick="editProduct({{ $product->id }})">Edit</button>
                        <button class="btn btn-sm" style="background:transparent; border:1px solid #17a2b8; color:#17a2b8; margin-left:0.5rem;" onclick="addStock({{ $product->id }})">+Stok</button>
                        <button class="btn btn-sm" style="background:transparent; border:1px solid #ffc107; color:#ffc107; margin-left:0.5rem;" onclick="reduceStock({{ $product->id }})">-Stok</button>
                        <button class="btn btn-sm" style="background:transparent; border:1px solid #dc3545; color:#dc3545; margin-left:0.5rem;" onclick="deleteProduct({{ $product->id }})">Hapus</button>
                    </td>
                </tr>
                @empty
                <tr>
                ...
                <div class="section-panel">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="tier-row" style="align-items: center; margin-bottom: 0;">
                            <label style="margin: 0; min-width: 60px;">Level {{ $i }}</label>
                            <input type="number" name="prices[{{ $i }}]" id="price_level_{{ $i }}" class="form-control" min="0" placeholder="Rp 0">
                        </div>
                        @endfor
                    </div>
                </div>
...
function addStock(productId) {
    ...
    document.getElementById('stockForm').action = '{{ route('admin.products.add-stock', ':id') }}'.replace(':id', productId);
}
...
function reduceStock(productId) {
    ...
    document.getElementById('stockAmount').value = '';
    document.getElementById('stockForm').action = '{{ route('admin.products.reduce-stock', ':id') }}'.replace(':id', productId);
}

function editProduct(productId) {
    fetch('{{ route('admin.products.edit', ':id') }}'.replace(':id', productId))
...
document.getElementById('price_level_1').value = product.prices?.find(p => p.price_level == 1)?.price || '';
                 ...
                 document.getElementById('productForm').action = '{{ route('admin.products.update', ':id') }}'.replace(':id', productId);
            }
        })
        .catch(err => {
...
document.getElementById('productForm').addEventListener('submit', function(e) {
    ...
    const url = productId ? 
        '{{ route('admin.products.update', ':id') }}'.replace(':id', productId) : 
...
document.getElementById('stockForm').addEventListener('submit', function(e) {
    ...
    const url = action === 'add' ? 
        '{{ route('admin.products.add-stock', ':id') }}'.replace(':id', productId) : 
        '{{ route('admin.products.reduce-stock', ':id') }}'.replace(':id', productId);
