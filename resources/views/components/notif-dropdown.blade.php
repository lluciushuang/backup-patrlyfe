<li class="nav-icon-wrapper">
    <a href="{{ route('notifications.index') }}" class="nav-icon-btn" title="Notifikasi">
    ...
    <div class="notif-dropdown" id="notifDropdown">
        <div class="notif-drop-header">
            ...
            <a href="{{ route('notifications.index') }}" style="font-size: 0.7rem; color: var(--orange); text-decoration: none; font-family: var(--font-mono);">LIHAT SEMUA</a>
        </div>
        <div class="notif-drop-list" id="notifList">
            @forelse($notifications as $notification)
            <a href="{{ $notification->is_read ? route('notifications.index') : '#' }}" class="notif-drop-item {{ $notification->is_read ? '' : 'unread' }}" onclick="{{ $notification->is_read ? '' : 'markAsRead(' . $notification->id . ')' }}">
                ...
                <div class="notif-drop-body">
                    <div class="notif-drop-title">{{ $notification->title }}</div>
                    <div style="font-size: 0.72rem; color: var(--gray-mid); margin-bottom: 0.2rem; line-height: 1.4;">{{ mb_substr($notification->message, 0, 50) }}</div>
                    <div class="notif-drop-time">{{ $notification->created_at->diffForHumans() }}</div>
                </div>
